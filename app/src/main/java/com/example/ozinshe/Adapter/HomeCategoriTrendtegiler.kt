...
class HomeCategoriTrendtegiler(val mItemClickListener: HomeCategoriTolukMult.ItemClickListener)  : RecyclerView.Adapter<HomeCategoriTrendtegiler.ViewHolder>() {
    ...
    inner class ViewHolder(private val binding: DizaineHomeMainMoviesBinding) :
        ...
        fun bindItem(itemMovieMain: Movy) {
            ...
            // и items содержит поле movies (вам может потребоваться адаптировать под вашу структуру)
                binding.TextHomeMoviesBold.text = itemMovieMain.name
                binding.TextHomeMoviesRegular.text = itemMovieMain.description
